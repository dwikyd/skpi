...
function Upload($folder, $sumber, $fupload_name){
  $vdir_upload = $folder;
  $vfile_upload = $vdir_upload . $fupload_name;
  $imageType = $_FILES["$sumber"]["type"];
  move_uploaded_file($_FILES["$sumber"]["tmp_name"], $vfile_upload);
}

function UploadFoto($folder, $sumber, $fupload_name){
  //direktori gambar
  $vdir_upload = $folder;
  $vfile_upload = $vdir_upload . $fupload_name;
  $imageType = $_FILES["$sumber"]["type"];
...
//  if($src_width>=350){
 // $dst_width = 350;
  ...
  imagedestroy($im);
  imagedestroy($im2);
  //return $vdir_upload . "small_" . $fupload_name;
}

function UploadLkd($fupload_name){
  //direktori gambar
  $vdir_upload = "../dokumenlkd/";
  $vfile_upload = $vdir_upload . $fupload_name;
