...
$kolom='id_jns_dokumen, nama_dokumen';
...
$totalData = mysqli_num_rows(mysqli_query($koneksi_skpi, "SELECT $kolom FROM jenis_dokumen $where"));
$totalFiltered = $totalData;

$query="SELECT $kolom FROM jenis_dokumen $where $order limit $offset, $limit";
...
while($p=mysqli_fetch_assoc($mhs)){
	...
	<a class="dropdown-item" href="#" data-toggle="modal" data-target="#modal-form" onclick="edit(\''.$p['id_jns_dokumen'].'\');"><span class="fas fa-edit mr-2"></span>Edit</a>
	<a class="dropdown-item text-danger" href="#" onclick="hapus(\''.$p['id_jns_dokumen'].'\');"><span class="fas fa-trash-alt mr-2"></span>Remove</a>
	</div>
	</div>';
	foreach ($columns as $k) {
