...
<?
$prodi=$_POST['prodi'];
$wisuda_ke=$_POST['wisuda'];
?>
<div class="card border-light shadow-sm mb-4">
	<div class="card-header">
		<h3><span class="fas fa-chart-line"></span> Progres SKPI</h3>
	</div>
	<div class="card-body">
		<?
		if(!isset($url[1])){ ?>
			...
			<div class="table-responsive py-4" id="content">
				<table class="table table-hover" id="data">
					<thead class="thead-light">
						<tr><th width="20px">No.</th><th width="80px">NIM</th><th>Nama</th><th>Prodi</th><th width="30px">Wisuda Ke</th><th width="30px">Prestasi</th><th width="30px">Aksi</th></tr>
					</thead>
					<tbody>
					</tbody>
				</table>
				...
				<table class="table table-bordered mb-0 rounded">
					...
					<tbody>
						...
						while($d=mysqli_fetch_assoc($sql)){
							...
							$aksi.="<div class='btn-group'><a href='file_prestasi/$d[nim]/$d[file]' target='_blank' class='btn btn-success btn-xs'><i class='fas fa-download'></i></a>$update</div>";

							if($d[$sts]=='Disetujui') $setuju++;
							echo "<tr><td width=70px>$no</td><td>$d[in_prestasi]<br><i>$d[eng_prestasi]</i>$catatan<td>".$d[$sts]."</td><td align=center nowrap>$aksi</td></tr>";
						}
						?>
					</tbody>
				</table>
		...
		elseif(isset($url[2])){
			include "pages/petugas/progres/form.php";
		}
		?>
	</div>
</div>
...
<script type="text/javascript" language="javascript" >
	function pilih(a){
		document.cookie = "nim="+a;
		window.open("sertifikat.html");
	}

	$(document).ready(function() {
		var prodi=$("#prodi").val();
		var wisuda=$("#wisuda_ke").val();
		var dataTable = $('#data').DataTable( {
			...
			"order": [[ 1, 'asc' ]],
			"ajax":{
				url: 'pages/petugas/progres/tabel.php?prodi='+prodi+'&wisuda_ke='+wisuda,
				type: "post",
				error: function(){
					$(".belum-error").html("");
	...
	function submit(a, b, c){
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				$.ajax({
					url: 'pages/petugas/progres/proses.php',
					type: "POST",
	...
	function batal(a){
		Swal.fire({
			title: 'Anda yakin ?',
			text: "SKPI NIM "+a+" akan dibatalkan dari Finalisasi ?",
			icon: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#3085d6',
			cancelButtonColor: '#d33',
