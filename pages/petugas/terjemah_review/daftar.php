...
<div>
	<form method="post" action="">
		<div class="form-group row" style="padding-bottom: 10px;">
			<label class="col-sm-2 control-label text-right">Reviewer </label>
			<div class="col-sm-10">
				<select class='form-control' name='id_pegawai' id="id_pegawai" onchange="submit();">
					<?
					if($_SESSION['admin_skpi']=="Y") echo "<option value=''>-- Semua Reviewer --</option>";
					...
					$sql="SELECT * FROM reviewer $where ORDER BY trim(nama_lengkap)";

					$jns=mysqli_query($koneksi_skpi, $sql);
					$jenis="";
					while($d=mysqli_fetch_assoc($jns)){
						$sel="";
						if($_POST['id_pegawai']== $d['id_pegawai']){
							// $_POST['jenis']=empty($_POST['jenis']) ? $d['jenis'] : $_POST['jenis'];
							$sel="selected";
						}
						$bahasa=$d['jenis']=="I" ? "Inggris" : "Arab";
						echo "<option value='$d[id_pegawai]' $sel>$d[nama_lengkap] ($bahasa)</option>";
					}
					?>
				</select>
			</div>
			<!-- <label class="col-sm-1 control-label text-right">Bahasa</label>
			<div class="col-sm-2">
				<select class='form-control' name="jenis" id="jenis" onchange="submit();">
					<option value="">-- Semua --</option>
					<?
					$jns=array("I", "A");
					foreach ($jns as $k) {
						$dk=$k=="I" ? "Inggris" : "Arab";
						$sel=$_POST['jenis']== $k ? "selected" : "";
...
<div class="table-responsive py-4">
	<table class="table table-hover table-striped table-bordered" id="data">
		<thead class="thead-light">
			<tr><th width="30px">No</th><th width="80px">Tanggal</th><th width="80px">NIM</th><th>Nama</th><th width="80px">Prodi</th><th width="30px">Terjemah</th><th width="30px">Aksi</th></tr>
		</thead>
		<tbody>
		</tbody>
	</table>
...
<script type="text/javascript" language="javascript" >
	...
	$(function(){
		...
		$('#data').DataTable({
			...
			"order": [[ 1, 'asc' ]],
			"ajax":{
				url: 'pages/petugas/terjemah_review/tabel.php?id_pegawai='+id_pegawai+'&kode_prodi='+prodi+"&terjemah="+trj,
				"dataType": "json",
				"type": "POST",
